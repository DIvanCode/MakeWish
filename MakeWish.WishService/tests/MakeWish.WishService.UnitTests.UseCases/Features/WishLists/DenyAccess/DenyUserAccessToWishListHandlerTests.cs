...
namespace MakeWish.WishService.UnitTests.UseCases.Features.WishLists.DenyAccess;

public class DenyUserAccessToWishListHandlerTests
{
    private readonly Mock<IUserContext> _userContextMock;
    private readonly IUnitOfWork _unitOfWork;
    private readonly DenyUserAccessToWishListHandler _handler;

    public DenyUserAccessToWishListHandlerTests()
    {
        _userContextMock = new Mock<IUserContext>();
        _unitOfWork = new UnitOfWorkStub();
        _handler = new DenyUserAccessToWishListHandler(_userContextMock.Object, _unitOfWork);
    }

    [Fact]
    public async Task Handle_ShouldDenyUserAccess_WhenUserIsOwner()
    {
        // Arrange
        ...
        var command = new DenyUserAccessToWishListCommand(wishList.Id, targetUser.Id);
    ...
    [Fact]
    public async Task Handle_ShouldReturnOk_WhenUserAlreadyHasNoAccess()
    {
        // Arrange
        var owner = new UserBuilder().Build();
        var targetUser = new UserBuilder().Build();
    ...
    public async Task Handle_ShouldReturnError_WhenUserIsNotAuthenticated()
    {
        ...
        var command = new DenyUserAccessToWishListCommand(Guid.NewGuid(), Guid.NewGuid());
    ...
    public async Task Handle_ShouldReturnError_WhenTargetUserNotFound()
    {
        ...
        var command = new DenyUserAccessToWishListCommand(wishList.Id, Guid.NewGuid());
