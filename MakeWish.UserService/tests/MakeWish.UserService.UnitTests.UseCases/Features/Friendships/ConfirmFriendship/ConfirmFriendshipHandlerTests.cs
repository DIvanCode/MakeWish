...
namespace MakeWish.UserService.UnitTests.UseCases.Features.Friendships.ConfirmFriendship;

public class ConfirmFriendshipHandlerTests
{
    private readonly Mock<IUserContext> _userContextMock;
    private readonly IUnitOfWork _unitOfWork;
    private readonly ConfirmFriendshipHandler _sut;

    public ConfirmFriendshipHandlerTests()
    {
        _userContextMock = new Mock<IUserContext>();
        _unitOfWork = new UnitOfWorkStub();
        _sut = new ConfirmFriendshipHandler(_unitOfWork, _userContextMock.Object);
    }
    ...
    public async Task Handle_UserNotAuthenticated_ReturnsAuthenticationError()
    {
        ...
        var command = new ConfirmFriendshipCommand(Guid.NewGuid(), Guid.NewGuid());
    ...
    [Fact]
    public async Task Handle_FirstUserTriesToConfirmFriendship_ReturnsForbiddenError()
    {
        // Arrange
        ...
        var command = new ConfirmFriendshipCommand(firstUser.Id, secondUser.Id);
    ...
    [Fact]
    public async Task Handle_SuccessfulFriendshipConfirmation_ReturnsFriendshipDto()
    {
        // Arrange
        var firstUser = new UserBuilder().Build();
        var secondUser = new UserBuilder().Build();
        ...
        Assert.Equivalent(firstUser, result.Value.FirstUser);
        Assert.Equivalent(secondUser, result.Value.SecondUser);
